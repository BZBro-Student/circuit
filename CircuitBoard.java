...
public class CircuitBoard {
	...
	public CircuitBoard(String filename) throws FileNotFoundException, InvalidFileFormatException {
		...
		try {
			// Checks first two tokens to see if they are int and assigns them to ROWS and
			// COLS
			...
			if (ROWS > 0 && COLS > 0) {
				int oneCount = 0;
				int twoCount = 0;
				// Goes through the file and adds each element to the array in the appropriate
				// index
				while (fileScan.hasNextLine()) {
					...
					for (int currColumn = 0; currColumn < COLS; currColumn++) {
						...
						// Counts start and end chars to ensure only one of each exist, also used to get
